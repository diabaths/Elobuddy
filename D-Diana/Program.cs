...
namespace D_Diana
{
    internal static class Program
    {
            ...
            foreach (AIHeroClient hero in EntityManager.Heroes.Enemies)
            {
                    ...
                    && target.IsValidTarget(Ignite.Range) && Ignite.IsReady())
                {
                    ...
                    if (_q.IsReady() && hero.IsValidTarget(_q.Range)
                        && _miscmenu["UseQKs"].Cast<CheckBox>().CurrentValue)
                    {
                        if (hero.Health <= qhDmg)
                        {
                            var predQ = _q.GetPrediction(hero);
                            if (predQ.HitChance >= HitChance.High) _q.Cast(hero);
                        }
                    }

                    if (_r.IsReady() && hero.IsValidTarget(_r.Range) && rRange
                        && _miscmenu["UseRKs"].Cast<CheckBox>().CurrentValue)
                    {
        ...
        private static void Misaya()
        {
            ...
            if (target.IsValidTarget(_e.Range) && !target.IsValidTarget(_w.Range) && useE && _e.IsReady()
                && !_w.IsReady())
            {
                _e.Cast();
            }
        ...
        private static void Combo()
        {
            ...
            if (target.IsValidTarget(_q.Range) && _combo["UseQCombo"].Cast<CheckBox>().CurrentValue && _q.IsReady())
            {
                ...
                if (predQ.HitChance >= HitChance.High) _q.Cast(predQ.CastPosition);
            }

            if (changetime >= 3000 && target.IsValidTarget(_r.Range)
                && _combo["UseRCombo"].Cast<CheckBox>().CurrentValue && _r.IsReady()
        ...
        private static void Harass()
        {
            ...
            if (changetimeh >= 3000 && /*!IsUnderTurret(target.ServerPosition) &&*/ target.IsValidTarget(_r.Range)
                && _harass["UseRHarass"].Cast<CheckBox>().CurrentValue && _r.IsReady()
                && (_qcreated == true || target.HasBuff("dianamoonlight")))
            {
                _r.Cast(target);
                ...
                if (_w.IsReady() && useW && mobs.IsValidTarget(_w.Range))
                {
                    if (minion.Count() >= 3)
                    {
                        _w.Cast();
                             ...
                             && mobs.Health < 0.75 * _player.GetSpellDamage(mobs, SpellSlot.W)) _w.Cast();
                }
        ...
        private static void JungleClear()
        {
            ...
            foreach (var minion in mininions)
            {
                ...
                if (changetime >= 3000 && _r.IsReady() && useR && minion.HasBuff("dianamoonlight")
                    && !mininions.Any(name => minion.Name.Contains("Mini")))
                {
                    _r.Cast(minion);
        ...
        private static void OnProcessSpellCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            ...
            try
            {
                ...
                if (args.Target.IsMe && _w.IsReady() && sender.IsEnemy && autoW) _w.Cast();
            }
